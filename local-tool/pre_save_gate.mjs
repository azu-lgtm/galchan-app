...
`保存前に以下を実行する:
  ...
  // ═══ 30. 🆕 過去動画被り検出（2026-05-17確定・azu指示「常に被らない内容にするルールいれろ。ゲートも」） ═══
  // 発端: 自ガル16構成案で4/22保存版動画(自ガル4+5+6+9まとめ)と同じ素材で提案→却下「内容は被らない方がいいよな。当たり前に」
  // 「ルールは書くだけでは守れない」原則違反受けて機械ゲートで恒久ブロック。
  // 3軸: タイトル核ワード / サムネ要素 / 構成素材（商品/トピック/フック）
  // 直近10本のいずれかと **3個以上一致** で FAIL。
  if (channel === 'galchan') {
    const { readFile: _rfMgmt } = await import('fs/promises');
    const mgmtPath = 'C:/Users/meiek/Dropbox/アプリ/remotely-save/obsidian/02_youtube/ガルちゃんねる/自分動画/動画管理リスト.md';

    // 過去動画リスト読み込み（パイプテーブル）
    let mgmtContent = '';
    try {
      mgmtContent = await _rfMgmt(mgmtPath, 'utf8');
    } catch (e) {
      fail('動画管理リスト.md 読込失敗', `${mgmtPath} が存在しないか読込不可。被りチェックには必須`);
    }

    // パイプテーブル行をパース: | 投稿日 | 台本名 | テーマ | タイトル | 台本リンク |
    ...
    // 4/22保存版動画も必ず含める（slice(-10)から漏れた場合）
    ...
    // 4/22「下記X本まとめ」型のscriptName保存版も補助検出
    ...
    if (recentVideos.length === 0) {
      pass('過去動画被り検出: 比較対象なし（スキップ）');
    } else {
      // 核ワード抽出関数: カタカナ/漢字/英数で分割し3文字以上のトークンを返す
      // 除外語（例外: 権威機関名/汎用語/型ワード）
      ...
      if (newTitle) {
        ...
        for (const past of recentVideos) {
          ...
          for (const w of newTitleWords) {
            // 完全一致 or 双方向部分一致（3文字以上のサブストリング一致）
      ...
      // ─── 軸2: サムネ被り（サムネ要素3個以上一致でFAIL） ───
      // 自分動画/サムネタイトル案/ または スプシ「サムネ文言」列のデータが過去動画にあれば理想だが、
      // 動画管理リストには無いので、ここでは「過去動画タイトルとサムネ文言の一致」で代用判定する。
      // payload.thumbnails (上段/下段/白枠4枚) の各要素を過去タイトルと照合し、共通核ワード3個以上で FAIL。
      ...
      // ─── 軸3: 構成素材被り（台本本文の商品/トピック/フックと過去動画タイトルが3個以上一致でFAIL） ───
      if (script && typeof script === 'string') {
        ...
        for (const past of recentVideos) {
          ...
          // 過去動画タイトル核ワードが台本本文に3個以上含まれる場合FAIL
          // （台本本文は大きいので閾値を厳しくする方向もあるが、azu指示「3個以上一致でFAIL」を厳守）
          if (matchCount >= 3) {
      ...
      if (fails.length > 0) {
        fail(`過去動画被り検出 ${fails.length}件: 直近10本との3軸チェック違反（azu指示2026-05-17）`,
          `${fails.join('\n')}\n\n→ 対策:\n` +
          `  1. ネタ出し段階で被り検知 → ネタ却下・別案提示\n` +
          `  2. 台本生成段階で被り検知 → 構成変更（商品/トピック/フック差し替え）\n` +
          `  3. 例外（被ってもOK）: 権威機関名/テーマタイプ/興味付け5要素（既にEXEMPT_WORDS除外済）\n` +
          `  詳細: DB/rules/ネタ出しルール.md「過去動画被り禁止」セクション\n` +
  ...
  if (channel === 'galchan' && script && typeof script === 'string') {
    ...
    // ─── ガード41: 商品名粒度ガード（2026-06-10 azu指示「商品をしっかり出す・競合と同等の粒度と精度で商品名を出す」） ───
    // 発端: 自ガル17「物価高で手放した/神商品」回で、軸が「やめた/手放した(行動)」に倒れ、
    //   本文が「美容院やめた・外食やめた・旅行減らした」等のカテゴリ/習慣だらけになり、
    //   具体的な商品名(固有名詞ブランド)が末尾の神商品ブロックにしか無い版を繰返し提出→azu激怒。
    // 競合実証: 40万再生「高いの→安いの正解」は全トピックが「商品A(具体名+価格)→商品B(具体名+価格)」の乗り換えペア。
    //   ぼかし(「近所の安い店」「普通の扇風機」)はNG。視聴者が買う/買わないを判断できる固有名詞が必須。
    // 「ルールは書くだけでは守れない」原則: 商品名粒度をゲート化しなかったから何度も戻った→恒久ブロック。
    // 詳細: memory/feedback_use_real_brand_names_competitor_validated.md
    {
      // 商品系テーマの動画のみ発動（更年期セルフケア等の非商品動画は対象外）
      const titleForTheme = (payload.title || materials.title || (typeof title === 'string' ? title : '') || '');
      const isProductTheme = /値上げ|物価高|買う|買って|買うな|神商品|手放|後悔|おすすめ|店|スーパー|商品|節約|安く/.test(titleForTheme + script.slice(0, 200));

      if (isProductTheme) {
        // 固有名詞ブランド/商品ホワイトリスト（競合40万動画+ステルス値上げ最新版+既存商品リストから抽出。azuが随時追加可）
        const BRAND_NAMES = [
          // 家電
          'ダイソン','バルミューダ','バイタミックス','ルンバ','デロンギ','ミラブル','パナソニック','アイリスオーヤマ','象印','タイガー','シャープ','山善','マキタ','ケルヒャー','ティファール','ソニッケアー','ドルツ','ヘルシオ','ホットクック','ブラウン','ネスカフェ','バルミューダ',
          // 寝具
          'テンピュール','西川','ニトリ','トゥルースリーパー','エアウィーヴ',
          // スーパー/PB/食品
          '業務スーパー','業ス','コストコ','カークランド','ロピア','トップバリュ','イオン','西友','大阪王将','味の素','ニチレイ','マルハニチロ','日清','明星','東洋水産','マルちゃん','エースコック','カルビー','湖池屋','明治','森永','ロッテ','ブルボン','グリコ','亀田製菓','ハウス','こくまろ','バーモント','キユーピー','雪印','よつ葉','キッコーマン','カルディ','もへじ','コスモス','ライフ','ガリガリ君','ミンティア','赤いきつね','きのこの山','サランラップ','クレラップ','ジップロック','チャルメラ','一平ちゃん','チキンラーメン','おかめ納豆','麺職人','サッポロ','アサヒ','キリン','サントリー','伊右衛門',
          // 飲料容器
          'サーモス',
          // 美容/化粧品
          'SK2','SK-II','無印','無印良品','アクアレーベル','エリクシール','キュレル','セザンヌ','キャンメイク','ちふれ','ニベア','ザーネ','ユスキン','メンソレータム','ケラチナミン','牛乳石鹸','ダヴ','ビオレ','ハトムギ','松山油脂','マツキヨ','アテニア','クラシエ','ファンケル','ハーバー','マキアージュ','エトヴォス','ディオール','リップモンスター','イヴサンローラン','サンローラン','滑らか本舗','豆乳イソフラボン','ロート','ビタ40','スマイル40','シエロ','サロンドプロ','ジョンマスター','ラックス','オクト','コスメデコルテ','ハーバー','まつきよ',
          // 日用品/洗剤
          'ダイソー','セリア','キャンドゥ','ドメスト','ファブリーズ','レノア','ダウニー','アタック','エリエール','ネピア','クリネックス',
          // 衣料
          'ユニクロ','GU','ワークマン','しまむら','セカスト','スタジオアリス',
          // 通信
          'ドコモ','楽天モバイル','ahamo','povo','LINEMO','mineo','日本通信','UQ','ワイモバイル','IIJ','イオンモバイル','ぷらら',
          // 飲食
          'サイゼリヤ','サイゼ','マクドナルド','ガスト','丸亀製麺','スターバックス','スタバ',
          // 食パン
          'ロイヤルブレッド','超熟','パスコ','彩や',
          // 油
          '米油','日清オイリオ','ボーソー',
          ...
          'ルイヴィトン','グッチ','シャネル','コーチ',
          // ドラッグ/その他
          'ウエルシア','マツモトキヨシ','ドンキ','カインズ',
        ];

        // 本文（スレ民/イッチ）行のみ抽出（ナレ/タイトル除外）
        const topicLines = [];
        for (const line of scriptLines) {
          ...
          if (!body) continue;
          if (speaker === 'ナレーション' || speaker === 'タイトル' || speaker === 'ナレ') continue;
          topicLines.push(body);
        }

        // ブランド出現カウント（全体 distinct + 3分割ごと distinct）
        const countBrands = (bodyArr) => {
          const found = new Set();
        ...
        const allBrands = countBrands(topicLines);

        const n = topicLines.length;
        const third = Math.max(1, Math.floor(n / 3));
        const seg1 = topicLines.slice(0, third);
        const seg2 = topicLines.slice(third, third * 2);
        const seg3 = topicLines.slice(third * 2);
        const b1 = countBrands(seg1), b2 = countBrands(seg2), b3 = countBrands(seg3);

        const MIN_DISTINCT = 25;   // 競合40万は約55ブランド。最低25を必須に
        const MIN_PER_THIRD = 3;   // 各1/3ブロックに最低3ブランド（末尾偏在＝カテゴリ止まりの検出）

        // 判定1: 全体の固有名詞商品数が少なすぎ
        if (allBrands.size < MIN_DISTINCT) {
          fail(`ガード41: 商品名(固有名詞)が競合より少なすぎ ${allBrands.size}件 < ${MIN_DISTINCT}件`,
            `検出ブランド: [${[...allBrands].join('/')}]\n` +
            `→ 競合40万動画は全トピックが「商品A(具体名+価格)→商品B(具体名+価格)」の乗り換えペアで約55ブランド。\n` +
            `  「美容院やめた」「外食やめた」等のカテゴリ/行動止まりを廃止し、具体的な商品名(固有名詞)に置換すること。\n` +
            `  ネガ商品名は「個人の感想」or 公的機関の警告でフレーム化して安全に出す。\n` +
            `  詳細: memory/feedback_use_real_brand_names_competitor_validated.md`);
        }

        // 判定2: 商品名の偏在（一部ブロックにブランドが無い＝カテゴリ止まり区間）
        const weakSegs = [];
        if (b1.size < MIN_PER_THIRD) weakSegs.push(`前半1/3=${b1.size}件[${[...b1].join('/')}]`);
        if (b2.size < MIN_PER_THIRD) weakSegs.push(`中盤1/3=${b2.size}件[${[...b2].join('/')}]`);
        if (b3.size < MIN_PER_THIRD) weakSegs.push(`後半1/3=${b3.size}件[${[...b3].join('/')}]`);
        if (weakSegs.length > 0) {
          fail(`ガード41: 商品名が偏在（${weakSegs.length}ブロックが固有名詞商品不足）`,
            `${weakSegs.join('\n')}\n` +
            `→ 商品名が一部(多くは末尾の神商品)に偏り、他ブロックはカテゴリ/行動止まり。\n` +
            `  全ブロックに具体的な商品名(乗り換えペア)を散りばめること。前半・中盤・後半すべてに最低${MIN_PER_THIRD}ブランド。\n` +
            `  詳細: memory/feedback_use_real_brand_names_competitor_validated.md`);
        }

        pass(`ガード41: 商品名粒度OK（固有名詞${allBrands.size}件・前${b1.size}/中${b2.size}/後${b3.size}）`);
      } else {
        pass('ガード41: 非商品テーマ動画のため商品名粒度チェック対象外');
      }
    }
  }
