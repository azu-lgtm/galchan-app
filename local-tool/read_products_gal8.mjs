...
const r = await sheets.spreadsheets.values.get({ spreadsheetId: SCRIPT_SS, range: '商品リスト!A1:K20' });
const rows = r.data.values || [];
console.log('=== 商品リストシート A1:K20 ===');
for (const [i, row] of rows.entries()) {
  const cells = (row || []).map((c, j) => `${String.fromCharCode(65 + j)}="${(c || '').slice(0, 35)}"`);
